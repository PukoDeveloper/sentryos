...
function renderProcessesTab() {
  ...
  for (var i = 0; i < processes.length; i++) {
    ...
    rows.push(OS.panel([
      OS.stack([
        ...
        OS.stack([
          OS.label('PID ' + p.pid, S.dimText),
          OS.button(isSel ? '已選擇' : '選擇', {
  ...
  if (rows.length === 0) {
    rows.push(OS.label('沒有執行中的程序', { fontSize: '13px', color: 'rgba(216,232,255,0.4)', textAlign: 'center', padding: '24px' }));
  }

  var actions = OS.stack([
    OS.button('終止程序', {
  ...
  var statusBar = OS.panel([
    OS.label(statusText, S.dimText)
  ], S.statusBar);

  return OS.stack([
    actions,
    OS.stack(rows, { gap: '6px', flex: '1', overflow: 'auto' }),
    statusBar,
...
function renderEventsTab() {
  var statsResult = OS.eventStats();
  var recentResult = OS.recentEvents(30);
  if (!statsResult.success) {
    return OS.label('無法取得事件資訊', { fontSize: '13px', color: '#ff6b6b', textAlign: 'center', padding: '24px' });
  }
  ...
  var summary = OS.panel([
    OS.stack([
      OS.stack([
        OS.label('事件類型', S.subText),
        OS.label('' + stats.length, { fontSize: '22px', color: '#67b8ff', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
      OS.stack([
        OS.label('總發射次數', S.subText),
        OS.label('' + totalEmits, { fontSize: '22px', color: '#a78bfa', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
      OS.stack([
        OS.label('活躍訂閱', S.subText),
        OS.label('' + totalSubs, { fontSize: '22px', color: '#6be68a', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
  ...
  for (var i = 0; i < stats.length; i++) {
    ...
    rows.push(OS.panel([
      OS.stack([
        OS.stack([
          OS.label(ev.event, { fontSize: '12px', color: '#d8e8ff', fontWeight: 'bold' }),
          progressBar(ratio, '#67b8ff', '4px'),
        ], { flex: '1', gap: '4px' }),
        OS.stack([
          OS.label('' + ev.emitCount + ' 次', { fontSize: '12px', color: '#a78bfa', textAlign: 'right' }),
          OS.label(ev.subscriberCount + ' 訂閱', { fontSize: '11px', color: 'rgba(216,232,255,0.5)', textAlign: 'right' }),
  ...
  if (rows.length === 0) {
    rows.push(OS.label('尚無事件紀錄', { fontSize: '12px', color: 'rgba(216,232,255,0.4)', textAlign: 'center', padding: '12px' }));
  }

  // Recent events timeline
  var timeline = [];
  timeline.push(OS.label('最近事件', S.sectionTitle));
  for (var i = 0; i < recent.length && i < 15; i++) {
    var r = recent[i];
    timeline.push(OS.stack([
      OS.label(fmtTime(r.timestamp), { fontSize: '11px', color: 'rgba(216,232,255,0.35)', fontFamily: 'monospace', minWidth: '64px' }),
      OS.label(r.event, { fontSize: '11px', color: '#67b8ff', flex: '1' }),
      OS.label(truncate(r.emitterAppId, 20), { fontSize: '11px', color: 'rgba(216,232,255,0.35)' }),
    ], { flexDirection: 'row', gap: '8px', padding: '3px 0' }));
  }
  if (timeline.length === 1) {
    timeline.push(OS.label('尚無紀錄', { fontSize: '11px', color: 'rgba(216,232,255,0.35)', padding: '8px 0' }));
  }

  return OS.stack([
    summary,
    OS.label('事件統計', S.sectionTitle),
    OS.stack(rows, { gap: '4px', flex: '1', overflow: 'auto' }),
    OS.stack(timeline, { gap: '0' }),
    OS.panel([OS.label('共 ' + stats.length + ' 種事件', S.dimText)], S.statusBar),
  ], { gap: '8px', flexDirection: 'column', flex: '1', padding: '8px 0' });
}

// ── API Tab ─────────────────────────────────────────────────
function renderApiTab() {
  var statsResult = OS.apiStats();
  var recentResult = OS.recentApiCalls(30);
  if (!statsResult.success) {
    return OS.label('無法取得 API 資訊', { fontSize: '13px', color: '#ff6b6b', textAlign: 'center', padding: '24px' });
  }
  var stats = statsResult.data;
  var recent = recentResult.success ? recentResult.data : [];
  ...
  var summary = OS.panel([
    OS.stack([
      OS.stack([
        OS.label('API 方法', S.subText),
        OS.label('' + stats.length, { fontSize: '22px', color: '#67b8ff', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
      OS.stack([
        OS.label('總呼叫次數', S.subText),
        OS.label('' + totalCalls, { fontSize: '22px', color: '#a78bfa', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
      OS.stack([
        OS.label('平均耗時', S.subText),
        OS.label(fmtMs(avgDuration), { fontSize: '22px', color: '#f5c542', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
    ], { flexDirection: 'row', justifyContent: 'space-around' }),
  ], S.cardHighlight);
  ...
  for (var i = 0; i < stats.length && i < 20; i++) {
    ...
    rows.push(OS.panel([
      OS.stack([
        OS.stack([
          OS.label(a.apiName + '.' + a.method, { fontSize: '12px', color: '#d8e8ff', fontWeight: 'bold', fontFamily: 'monospace' }),
        ], { flex: '1' }),
        OS.stack([
          OS.label('' + a.callCount + ' 次', { fontSize: '12px', color: '#a78bfa', textAlign: 'right' }),
          OS.label('avg ' + fmtMs(a.avgDuration), { fontSize: '11px', color: '#f5c542', textAlign: 'right' }),
        ], { alignItems: 'flex-end', gap: '2px', minWidth: '100px' }),
      ], { flexDirection: 'row', alignItems: 'center', gap: '12px' }),
    ], { padding: '8px 12px', borderRadius: '6px', background: 'rgba(255,255,255,0.02)' }));
  }
  if (rows.length === 0) {
    rows.push(OS.label('尚無 API 呼叫紀錄', { fontSize: '12px', color: 'rgba(216,232,255,0.4)', textAlign: 'center', padding: '12px' }));
  }

  // Recent API calls
  var timeline = [];
  timeline.push(OS.label('最近呼叫', S.sectionTitle));
  for (var i = 0; i < recent.length && i < 15; i++) {
    var r = recent[i];
    var callColor = r.success ? '#6be68a' : '#ff6b6b';
    timeline.push(OS.stack([
      OS.label(fmtTime(r.timestamp), { fontSize: '11px', color: 'rgba(216,232,255,0.35)', fontFamily: 'monospace', minWidth: '64px' }),
      OS.label(r.apiName + '.' + r.method, { fontSize: '11px', color: '#67b8ff', flex: '1', fontFamily: 'monospace' }),
      OS.label(fmtMs(r.duration), { fontSize: '11px', color: callColor, minWidth: '50px', textAlign: 'right' }),
    ], { flexDirection: 'row', gap: '8px', padding: '3px 0' }));
  }
  if (timeline.length === 1) {
    timeline.push(OS.label('尚無紀錄', { fontSize: '11px', color: 'rgba(216,232,255,0.35)', padding: '8px 0' }));
  ...
  return OS.stack([
    summary,
    OS.label('API 呼叫排行', S.sectionTitle),
    OS.stack(rows, { gap: '4px', flex: '1', overflow: 'auto' }),
    OS.stack(timeline, { gap: '0' }),
    OS.panel([OS.label('共 ' + stats.length + ' 個 API 方法  |  ' + totalCalls + ' 次呼叫', S.dimText)], S.statusBar),
  ], { gap: '8px', flexDirection: 'column', flex: '1', padding: '8px 0' });
}

// ── Permissions Tab ─────────────────────────────────────────
function renderPermissionsTab() {
  var result = OS.permissionStats();
  if (!result.success) {
    return OS.label('無法取得權限資訊', { fontSize: '13px', color: '#ff6b6b', textAlign: 'center', padding: '24px' });
  }
  ...
  var summary = OS.panel([
    OS.stack([
      OS.stack([
        OS.label('總檢查次數', S.subText),
        OS.label('' + data.totalChecks, { fontSize: '22px', color: '#67b8ff', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
      OS.stack([
        OS.label('拒絕次數', S.subText),
        OS.label('' + data.totalDenied, { fontSize: '22px', color: '#ff6b6b', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
      OS.stack([
        OS.label('拒絕率', S.subText),
        OS.label(denyRate + '%', { fontSize: '22px', color: denyRate > 20 ? '#ff6b6b' : '#6be68a', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
    ], { flexDirection: 'row', justifyContent: 'space-around' }),
  ], S.cardHighlight);
  ...
  // Per-app breakdown
  var appRows = [];
  appRows.push(OS.label('依應用程式', S.sectionTitle));
  var appKeys = [];
  ...
  for (var i = 0; i < appKeys.length && i < 15; i++) {
    ...
    appRows.push(OS.panel([
      OS.stack([
        OS.label(truncate(appId, 28), { fontSize: '12px', color: '#d8e8ff', flex: '1', fontFamily: 'monospace' }),
        OS.label('' + entry.checks + ' 次', { fontSize: '12px', color: '#a78bfa', minWidth: '60px', textAlign: 'right' }),
        OS.label(entry.denied + ' 拒絕', { fontSize: '12px', color: dColor, minWidth: '60px', textAlign: 'right' }),
      ], { flexDirection: 'row', alignItems: 'center', gap: '8px' }),
    ], { padding: '6px 12px', borderRadius: '6px', background: 'rgba(255,255,255,0.02)' }));
  }
  if (appKeys.length === 0) {
    appRows.push(OS.label('尚無紀錄', { fontSize: '12px', color: 'rgba(216,232,255,0.4)', textAlign: 'center', padding: '12px' }));
  }

  // Per-permission breakdown
  var permRows = [];
  permRows.push(OS.label('依權限類型', S.sectionTitle));
  var permKeys = [];
  ...
  for (var i = 0; i < permKeys.length && i < 15; i++) {
    ...
    permRows.push(OS.panel([
      OS.stack([
        OS.label(truncate(perm, 32), { fontSize: '11px', color: '#d8e8ff', flex: '1', fontFamily: 'monospace' }),
        OS.label('' + pe.checks, { fontSize: '11px', color: '#a78bfa', minWidth: '40px', textAlign: 'right' }),
        OS.label(pe.denied > 0 ? pe.denied + ' ✗' : '✓', { fontSize: '11px', color: pdColor, minWidth: '40px', textAlign: 'right' }),
      ], { flexDirection: 'row', alignItems: 'center', gap: '6px' }),
    ], { padding: '4px 12px', borderRadius: '4px', background: 'rgba(255,255,255,0.01)' }));
  }
  if (permKeys.length === 0) {
    permRows.push(OS.label('尚無紀錄', { fontSize: '11px', color: 'rgba(216,232,255,0.35)', padding: '8px 0' }));
  }

  return OS.stack([
    summary,
    OS.stack(appRows, { gap: '4px', flex: '1', overflow: 'auto' }),
    OS.stack(permRows, { gap: '2px', maxHeight: '160px', overflow: 'auto' }),
    OS.panel([OS.label('應用程式: ' + appKeys.length + '  |  權限類型: ' + permKeys.length, S.dimText)], S.statusBar),
  ], { gap: '8px', flexDirection: 'column', flex: '1', padding: '8px 0' });
}
...
function renderPerformanceTab() {
  ...
  if (!snapResult.success) {
    return OS.label('無法取得效能資訊', { fontSize: '13px', color: '#ff6b6b', textAlign: 'center', padding: '24px' });
  }
  ...
  var summary = OS.panel([
    OS.stack([
      OS.stack([
        OS.label('運行時間', S.subText),
        OS.label(fmtUptime(snap.uptime), { fontSize: '20px', color: '#67b8ff', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
      OS.stack([
        OS.label('總執行次數', S.subText),
        OS.label('' + perf.totalExecutions, { fontSize: '20px', color: '#a78bfa', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
      OS.stack([
        OS.label('平均執行時間', S.subText),
        OS.label(fmtMs(perf.avgExecutionTime), { fontSize: '20px', color: '#f5c542', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
    ], { flexDirection: 'row', justifyContent: 'space-around' }),
  ], S.cardHighlight);

  // Process lifecycle overview
  var procCards = OS.panel([
    OS.stack([
      OS.stack([
        OS.label('已啟動', S.subText),
        OS.label('' + proc.totalLaunched, { fontSize: '18px', color: '#6be68a', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
      OS.stack([
        OS.label('已終止', S.subText),
        OS.label('' + proc.totalTerminated, { fontSize: '18px', color: '#ff6b6b', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
      OS.stack([
        OS.label('執行中', S.subText),
        OS.label('' + proc.activeProcesses, { fontSize: '18px', color: '#67b8ff', fontWeight: 'bold' }),
      ], { alignItems: 'center' }),
    ], { flexDirection: 'row', justifyContent: 'space-around' }),
  ], S.card);
  ...
  var appRows = [];
  appRows.push(OS.label('應用程式統計', S.sectionTitle));
  var appKeys = [];
  ...
  for (var i = 0; i < appKeys.length; i++) {
    ...
    appRows.push(OS.panel([
      OS.stack([
        OS.label(truncate(appId, 24), { fontSize: '12px', color: '#d8e8ff', flex: '1' }),
        OS.label('啟動 ' + ap.launched, { fontSize: '11px', color: '#6be68a', minWidth: '50px', textAlign: 'right' }),
        OS.label('終止 ' + ap.terminated, { fontSize: '11px', color: '#ff6b6b', minWidth: '50px', textAlign: 'right' }),
        OS.label(active > 0 ? '●' + active : '', { fontSize: '11px', color: '#67b8ff', minWidth: '30px', textAlign: 'right' }),
      ], { flexDirection: 'row', alignItems: 'center', gap: '6px' }),
    ], { padding: '6px 12px', borderRadius: '6px', background: 'rgba(255,255,255,0.02)' }));
  }
  ...
  // Recent executions
  var recentRows = [];
  recentRows.push(OS.label('最近執行紀錄', S.sectionTitle));
  ...
  for (var i = 0; i < recExec.length && i < 12; i++) {
    ...
    recentRows.push(OS.stack([
      OS.label(fmtTime(ex.timestamp), { fontSize: '11px', color: 'rgba(216,232,255,0.35)', fontFamily: 'monospace', minWidth: '64px' }),
      OS.label('PID ' + ex.pid, { fontSize: '11px', color: 'rgba(216,232,255,0.5)', minWidth: '50px' }),
      OS.panel([
        OS.panel([], {
      ...
      OS.label(fmtMs(ex.duration), { fontSize: '11px', color: durColor, minWidth: '55px', textAlign: 'right', fontFamily: 'monospace' }),
    ], { flexDirection: 'row', gap: '8px', padding: '2px 0', alignItems: 'center' }));
  }

  // System totals
  var systemInfo = OS.stack([
    OS.label('事件: ' + snap.events.totalEmits + ' 次', S.dimText),
    OS.label('API: ' + snap.api.totalCalls + ' 次', S.dimText),
    OS.label('權限: ' + snap.permissions.totalChecks + ' 次 (' + snap.permissions.totalDenied + ' 拒絕)', S.dimText),
  ], { flexDirection: 'row', gap: '16px' });

  return OS.stack([
    summary,
    procCards,
    OS.stack(appRows, { gap: '4px' }),
    OS.stack(recentRows, { gap: '0', flex: '1', overflow: 'auto' }),
    OS.panel([systemInfo], S.statusBar),
  ], { gap: '8px', flexDirection: 'column', flex: '1', padding: '8px 0' });
}

// ── Storage Tab ─────────────────────────────────────────────
function renderStorageTab() {
  var usageResult = OS.storageUsage();
  if (!usageResult.success) {
    return OS.panel([
      OS.label('無法取得儲存空間資訊: ' + (usageResult.error || '未知錯誤'), {
  ...
  for (var i = 0; i < tierKeys.length; i++) {
    ...
    cards.push(OS.panel([
      OS.stack([
        OS.stack([
          OS.label(tierNames[k], { fontSize: '13px', color: tierColors[k], fontWeight: 'bold' }),
          OS.label(t.used + ' / ' + t.capacity + ' 筆', S.subText),
        ], { gap: '4px', flex: '1' }),
        OS.label(ratio + '%', { fontSize: '20px', color: bColor, fontWeight: 'bold', width: '60px', textAlign: 'right' }),
      ], { flexDirection: 'row', alignItems: 'center', gap: '12px' }),
  ...
  var total = OS.panel([
    OS.stack([
      OS.label('總容量', { fontSize: '13px', color: '#d8e8ff', fontWeight: 'bold' }),
      OS.label(totalUsed + ' / ' + totalCap + ' 筆  (' + totalRatio + '%)', S.subText),
  ...
  return OS.stack([
    total,
    OS.stack(cards, { gap: '8px' }),
    OS.panel([
      OS.label('資料項目總數: ' + data.totalEntries, S.dimText)
    ], S.statusBar),
