...
SentryOS 支援非同步（async/await）Host API。Host 端函式若回傳 `Promise`，`ScriptRuntime` 的橋接層會自動建立對應的 `QuickJSDeferredPromise`，並在 Promise 結算後透過 `executePendingJobs()` 驅動沙箱微任務隊列。

**沙箱程式碼使用方式**：

```javascript
// async/await（需在 async 函式內）
async function fetchData() {
  var result = await OS.network.requestAsync('https://api.example.com/data');
  if (result.success) {
    OS.console.log(result.data.body);
  }
}
fetchData();

// 或使用 .then()
OS.network.requestAsync('https://api.example.com/data').then(function(result) {
  OS.console.log(result.data.body);
});
```

**注意事項**：
- 非同步 API 不受 300 ms 執行逾時限制；逾時只計算同步程式碼的執行時間。
- 同步 API（`OS.network.request`）仍然可用，適合不需要等待的簡單場景。
- 若應用程式程序在 Promise 結算前被終止，橋接層會安全地丟棄該結果（不會觸發任何回呼）。
...
非同步 HTTP 請求，使用 `fetch()` 進行真正的非同步 I/O。需要 `network.request` 權限。
支援 `await` 語法，不佔用同步執行逾時。
...
async function loadData() {
  ...
  if (result.success) {
    OS.console.log('Response: ' + result.data.body);
  }
}
loadData();
```

> **注意**：URL 必須在系統允許清單中，否則回傳 `NotAllowed`。超時預設為 10,000 ms。

### request(url, options?)

發送同步 HTTP 請求。需要 `network.request` 權限。

```javascript
var result = OS.network.request('https://api.example.com/data', {
  method: 'POST',                      // 預設 'GET'
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ key: 'value' }),
  timeout: 10000,                      // ms
