...
|------|------|------|---------------|
| `Window` | GUI 應用，透過 `OS` API 建立視窗 | 是 | 否 |
...
```javascript
// main.js
OS.publishHealth({ status: 'ok', uptime: 0 });

// 可使用事件 API 與其他程式溝通
OS.emit('my-service.started', { pid: OS.pid });
```
...
Console 應用可透過 Shell API 實現進階系統指令：
...
Library 不面向使用者，開機時自動載入（init → 快取 → 程序結束）。其他程式可透過 `OS.loadLibrary()` 載入其程式碼。
...
完整權限列表請參閱 [Host API 參考 — 權限速查表](./host-api.md#權限速查表)。
